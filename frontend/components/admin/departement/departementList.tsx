...
import { Department, DepartmentStatus } from "@/lib/api/services"
...
export default function DepartmentCards() {
  ...
  const {
    ...
    enabled: true,
    onSuccess: (data) => {
      console.log(`✅ ${data.departments.length} départements chargés`)
    },
    onError: (error) => {
  ...
  const handleToggleStatus = async (departmentId: string, currentStatus: string) => {
    try {
      await departmentService.toggleDepartmentStatus(departmentId, currentStatus as any)
  ...
  // Extraire les bâtiments uniques pour le filtre
  const buildings = Array.from(new Set(departmentsData?.departments.map(dept => dept.batiment) || []))

  // Statistiques
  const stats = [
    {
      title: "Total Départements",
      value: departmentsData?.stats?.total?.toString() || "0",
      icon: Building2,
    ...
    },
    {
      ...
      value: departmentsData?.stats?.totalEnseignants?.toString() || "0",
      icon: Users,
    ...
    },
    {
      ...
      value: departmentsData?.stats?.totalEtudiants?.toString() || "0",
      icon: GraduationCap,
  ...
  // Budget total formaté
  const budgetTotal = departmentsData?.stats?.budgetTotal 
    ? departmentService.formatBudget(departmentsData.stats.budgetTotal)
    : "0 €"
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Gérez tous les départements de l'établissement
          </p>
        </div>
      ...
      <div className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl p-6 text-white">
        <div className="flex flex-col md:flex-row justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold mb-2">Budget total annuel</h2>
            <p className="text-blue-100">Tous départements confondus</p>
          </div>
              ...
              <Card key={dept.id} className="p-6 border border-border bg-card hover:shadow-md transition-shadow">
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-start gap-4">
                    <div 
                      ...
                      style={{ 
                        backgroundColor: dept.couleur || '#8b5cf6',
                        background: dept.couleur 
                          ? `linear-gradient(135deg, ${dept.couleur}80, ${dept.couleur})`
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="text-lg font-semibold text-foreground">{dept.nom}</h3>
                        <Badge className={statusColor}>
                          {statusLabel}
                        </Badge>
                      </div>
                      <p className="text-sm text-muted-foreground">
                        Code: <span className="font-mono">{dept.code}</span> • Dirigé par {dept.directeurNom || dept.directeur}
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">
                        {dept.batiment} {dept.etage && `- Étage ${dept.etage}`}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => handleToggleStatus(dept.id, dept.status)}>
                        ...
                        <DropdownMenuItem 
                          ...
                          onClick={() => handleDeleteDepartment(dept.id, dept.nom)}
                        >
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  <div className="flex flex-col items-center p-4 rounded-lg bg-muted/50">
                    ...
                    <p className="text-2xl font-bold text-foreground">{dept.enseignantsCount}</p>
                    <p className="text-xs text-muted-foreground">Enseignants</p>
                  </div>
                  <div className="flex flex-col items-center p-4 rounded-lg bg-muted/50">
                    <GraduationCap className="h-5 w-5 text-purple-600 mb-2" />
                    <p className="text-2xl font-bold text-foreground">{dept.etudiantsCount}</p>
                ...
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-border">
                  <div>
                    ...
                    <span className="text-lg font-semibold text-foreground">
                      {departmentService.formatBudget(dept.budgetAnnuel)}
                    </span>
                  </div>
      ...
      {departmentsData && (
        <div className="text-center text-sm text-muted-foreground">
          Affichage de {departmentsData.departments.length} sur {departmentsData.pagination.total} départements
