...
import { LoginCredentials } from '@/lib/api/services';
...
export function LoginForm({
  ...
  // Récupérer la redirection depuis les query params ou props
  const redirectFromParams = searchParams.get('redirect');
  const finalRedirectTo = redirectTo || redirectFromParams || '/dashboard';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (onSuccess) {
        ...
        router.refresh(); // Rafraîchir pour mettre à jour le contexte
      }
      ...
      } else if (err.message.includes('compte désactivé')) {
        setError('Votre compte a été désactivé. Contactez l\'administrateur.');
      } else if (err.message.includes('non vérifié')) {
  ...
  /**
   * REMPLIR AVEC UN COMPTE DE TEST (pour développement)
   */
    ...
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      ...
      <Card className="border-2 shadow-lg">
        ...
        <CardContent>
          ...
          {/* Bouton de test (uniquement en développement) */}
          {process.env.NODE_ENV === 'development' && (
            <div className="mb-4">
              ...
              >
                Remplir avec un compte de test
              </Button>
            </div>
          )}
      ...
      {/* Mode débug (uniquement en développement) */}
      {process.env.NODE_ENV === 'development' && (
        <div className="mt-4 p-4 bg-muted rounded-lg text-xs">
          <div className="font-semibold mb-2">Debug Info:</div>
          <div>Redirection: {finalRedirectTo}</div>
